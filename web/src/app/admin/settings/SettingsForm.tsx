...
export function SettingsForm() {
  ...
  function handleSetGlobalChatBackground(backgroundId: string) {
    updateSettingField([
      {
        fieldName: "chat_background",
        newValue: backgroundId === CHAT_BACKGROUND_NONE ? null : backgroundId,
      },
    ]);
  }
  ...
  return (
    <>
      ...
      <label className="flex flex-col text-sm mb-4">
        ...
        <SubLabel>
          Set a workspace-wide chat background for all users.
        </SubLabel>
        <div className="mt-2 grid w-full max-w-3xl grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
          {CHAT_BACKGROUND_OPTIONS.map((background) => {
            const isSelected =
              (settings.chat_background ?? CHAT_BACKGROUND_NONE) ===
              background.id;
            ...
            return (
              <button
                key={background.id}
                type="button"
                onClick={() => handleSetGlobalChatBackground(background.id)}
                className={cn(
                  "rounded-12 border p-1 text-left transition-colors",
                  isSelected
                    ? "border-theme-primary-05 ring-2 ring-theme-primary-05"
                    : "border-border-02 hover:border-border-03"
                )}
              >
                {background.thumbnail === CHAT_BACKGROUND_NONE ? (
                  <div className="flex h-20 w-full items-center justify-center rounded-08 bg-background-neutral-02">
                    <span className="text-text-03 text-sm">No Background</span>
                  </div>
                ) : (
                  <Image
                    ...
                    width={320}
                    height={80}
                    className="h-20 w-full rounded-08 object-cover bg-background-neutral-02"
                  />
                )}
                <span className="block px-1 py-1 text-text-01 text-sm">
                  {background.label}
                </span>
              </button>
            );
      ...
      <Checkbox
        ...
        sublabel="If set, users will not be required to sign in to use StarwoodGPT."
          ...
          <Modal.Content>
            ...
            <Modal.Body>
              <p>
                ...
                anyone to use StarwoodGPT without signing in.
              </p>
            </Modal.Body>
            <Modal.Footer>
      ...
      {isEnterpriseEnabled && (
        <>
          ...
          <IntegerInput
            ...
            sublabel="Enter the maximum number of days you would like StarwoodGPT to retain chat messages. Leaving this field empty will cause StarwoodGPT to never delete chat messages."
